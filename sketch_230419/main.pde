...
int numOfCells_w = 100;
int numOfCells_h = 100;
...
void setup()
{
  ...
  img = loadImage("cat_sq.png"); // 이미지 로드
...
void draw() {
  ...
  {
  case SA:
    drawStyleSA(mosaic_colors, numOfCells_w, numOfCells_h, cellSize_w, cellSize_h);
    break;

  case CHOI:
    drawStyleCHOI(mosaic_colors, numOfCells_w, numOfCells_h, cellSize_w, cellSize_h);
    break;

  case HAN:
    drawStyleHAN(mosaic_colors, numOfCells_w, numOfCells_h, cellSize_w, cellSize_h);
    break;
    
  case LEE:
    drawStyleLEE(mosaic_colors, numOfCells_w, numOfCells_h, cellSize_w, cellSize_h);
    break;
...
void keyPressed()
{
  switch(key)
  {
    ...
    case '2':
    state = StyleState.HAN;
    break;

    case '3':
    state = StyleState.CHOI;
    break;
    
    case '4':
    state = StyleState.LEE;
    break;
  }
}
